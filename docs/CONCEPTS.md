...
**The fundamental idea**: Different query types benefit from different retrieval strategies. A simple factual lookup ("What is the capital of France?") works best with BM25 keyword search, while a complex analytical question ("Compare the economic policies of the last three presidents") needs multi-document synthesis. The Hierarchical RAG Router classifies incoming queries by complexity and **names** the retrieval strategy that suits each one: simple keyword queries to BM25, factual queries to dense vector retrieval, multi-hop queries requiring relationship traversal to Graph RAG, and analytical queries requiring hierarchical summarization to RAPTOR. Note that it names rather than dispatches: `route()` returns the strategy as a string and the caller wires it to a retriever. For the dense case that retriever is `VectorDbRetrieval`, which implements `RetrievalCallback` over any vector backend. The classifier uses lightweight heuristics (query length, question type, entity count) to avoid adding an LLM call to the critical path.
...
## 234. Precise Tokenization

Model-aware token counting with 3 precision levels: `TokenPrecision::Approximate` (~3.5 chars/token, ±15-20%, reserve 20%), `TokenPrecision::Bpe` (200-rule BPE, ±10-15%, reserve 15%), `TokenPrecision::Precise` (tiktoken exact, ±0-1%, reserve 10%). `ProviderTokenCounter` dispatches automatically: OpenAI models use tiktoken (if `precise-tokens` feature), all cloud models use BPE, local/unknown use heuristic. `AiAssistant::estimate_tokens_for_current_model()` and `token_precision()` provide the API. `build_allocated_context()` uses model-aware counting for all budget decisions with dynamic response reserve. Feature flag: `precise-tokens` (optional, adds ~4MB for tiktoken vocabulary tables). Security: max 1MB input for tiktoken (DoS prevention), OnceCell caching per model family.

## 235. Topic-Aware RAG Relevance

Solves the "Mordor vs Alps" problem: keyword-relevant but topic-irrelevant chunks. Five features in `RagFeatures` (33→38): `topic_matching` (Jaccard keyword overlap, zero cost, from Semantic tier — penalizes off-topic chunks with configurable penalty 0.2×), `autocut` (detect natural gap in score distribution and discard below elbow, zero cost, from Semantic tier), `topic_matching_llm` (LLM classifies ambiguous chunks as ON_TOPIC/PARTIAL/OFF_TOPIC in batches of 10, only processes Jaccard "Partial" chunks, from Thorough tier), `self_query_filter` (LLM extracts structured section/date/keyword filters from query before searching, inspired by LangChain Self-Query, from Thorough tier), `chunk_granular_scoring` (ChunkRAG-style sentence-level relevance within chunks, extracts only relevant sentences, from Agentic tier). `TopicMatcher` uses language-aware stopwords (EN+ES+code keywords), Jaccard similarity for overlap, configurable thresholds. `TopicMatchConfig` serializable for tier definitions. Novel contribution: lightweight topic matching without LLM as pre-filter — no other project does this.

## 236. Audio Effect Chain & Speaker Verification

`AudioEffectChain` processes audio through ordered effects: `NoiseGate` (RMS threshold, ~50us), `AutoGainControl` (target dB normalization, ~50us), `AcousticEchoCanceller` (NLMS adaptive filter, ~5000us), `Compressor` (dynamic range with configurable ratio, ~100us), `Distortion` (hard clipping, ~50us), `Reverb` (Schroeder 4-comb filter, ~5000us). Each effect implements `AudioEffect` trait with `process(&mut [i16], sample_rate)`, `name()`, `is_enabled()`, `estimated_latency_us()`. Chain tracks total latency and provides per-effect breakdown. `MfccSpeakerVerifier`: pure-Rust MFCC computation (13 coefficients from frame energy, zero-crossings, spectral moments, cosine basis) for ~85% speaker identification accuracy without external models. `SpeakerGate` manages multi-profile enrollment, identification with confidence threshold, owner-only mode, and active speaker tracking for GUI.

## 237. Voice Cloning Infrastructure

`VoiceCloneProvider` trait abstracts voice cloning across providers: `enroll(audio, name) → ClonedVoiceId`, `synthesize_cloned(text, voice_id, options) → SynthesisResult`, `list_cloned_voices()`, `delete_cloned_voice()`. Two implementations: `ElevenLabsCloneProvider` (cloud, Instant Voice Clone API with multipart upload, ~1min audio needed) and `XttsCloneProvider` (local Coqui XTTS v2 HTTP server, reference audio at synthesis time, ~6s audio needed). `assess_enrollment_quality()` checks duration (min 3s), RMS level, silence ratio — rejects below 30% quality. `ClonedVoiceProfile` stores voice_id, provider, name, creation time, quality score, consent flag.

## 238. Speaker Diarization

`SpeakerDiarizer` auto-detects distinct speakers without enrollment. Uses MFCC embeddings clustered in real-time: each new voice that doesn't match existing clusters (above similarity threshold) creates a new "Speaker N" label. Running mean embedding updated per segment for cluster adaptation. Configurable threshold (default 0.55) and max_clusters (default 10) to prevent unbounded growth. `DiarizationResult` enum: `Assigned { label, index, confidence }`, `NewSpeaker { label, index }`, `Inconclusive`. At capacity, forces assignment to closest cluster. `reset()` clears all clusters for fresh sessions.

## 239. Audio Model Registry

`AudioModelRegistry` provides a catalog of known audio models (Whisper STT variants, Piper TTS voices, XTTS v2) with download management. `AudioModelInfo`: name, category (STT/TTS/VoiceClone/Emotion), size estimate, URL, SHA-256 checksum, filename. `download_model()` streams from HuggingFace with progress callback and post-download checksum verification. `detect_installed()` scans model directory for known filenames. Platform-appropriate storage: `%LOCALAPPDATA%\ai_assistant\models` (Windows), `~/.cache/ai_assistant/models` (Linux/macOS), override via `AI_ASSISTANT_MODEL_DIR` env var. Pure-Rust SHA-256 hasher (verified against RFC test vectors).

## 240. ai_virtual_mic — Voice Transform Application

Desktop GUI application (egui/eframe) for real-time audio processing with 5 modes: **Monitor** (VU meter + diarization), **Passthrough** (effects chain only), **Transform** (STT→TTS with voice cloning), **Direct** (same, normal speaker output), **Agent** (full pipeline: STT → mood → AiAssistant with RAG → mood-aware TTS → output). Architecture: cpal input → lock-free ring buffer (ringbuf) → processing thread → cpal output. Features: device enumeration/selection, noise gate + AGC sliders, latency calculator (current/avg/min/max with VU bar), Discord AI Mode preset (auto-detect VB-Cable/VoiceMeeter), speaker diarization/identification panels with enrollment, audio model catalog with download/delete, config persistence (JSON).

## 241. Agent Mode — Voice AI Participant

In Agent mode, ai_virtual_mic becomes a voice participant in conversations. Agent has a configurable name (default "Luna"), `respond_only_when_addressed` filter (detects name mentions + contextual patterns like "hey", "oye", "dime" in EN/ES), and `wait_for_silence_ms` (default 800ms) to avoid talking over people. Diarization labels ("Speaker 1") are auto-resolved to real names when someone says "soy Carlos", "I'm John", etc. via `extract_self_introduction()` (10+ patterns). The LLM receives prompts like "[Carlos (Speaker 1) says:] hola Luna" so it can address people by name. Connection modes: Local (Ollama), Remote Node (ai_assistant_server API), Direct Provider (OpenAI/Anthropic without RAG), Custom. Presets with one-click setup. Pipeline status bar: Listening → STT → LLM → Wait → TTS. Chat view with mood badges and speaker labels.

## 242. LLM Enhancement Infrastructure

`LlmEnhancer` trait provides a uniform interface for optional LLM calls across 17 modules. Pattern: each module has a heuristic baseline (always works, zero cost). When `llm_enhanced: true` in config AND an `LlmEnhancer` is available, the module builds a prompt (`build_*_prompt()`), sends to LLM, parses response (`parse_*_response()` with `extract_json()`), and merges with the heuristic result. If the LLM fails, the heuristic result is returned (graceful fallback). Security: `prompt_wrap()` wraps user content in delimiters with explicit "treat as DATA" instructions. `MockLlm` and `FailingMockLlm` enable testing both paths. The trait is minimal: `generate(prompt, max_tokens) → Result<String>` + `model_name()` + `is_available()`.

## 243. LLM-Enhanced Pipeline (17 Modules)

Seventeen modules enhanced with optional LLM calls: (1) Conversation compaction → abstractive summarization, (2) Entity extraction → NER for person/org/location/date/concept, (3) Intent classification → semantic intent + confidence, (4) Query expansion → LLM paraphrasing (wired existing scaffold), (5) Response quality → relevance/coherence/completeness scoring, (6) Topic detection → LLM classification merged with keyword baseline, (7) Auto-model selection → task type + complexity, (8) Guardrail evaluation → injection detection (defense-in-depth: can upgrade safe→detected, never downgrade), (9) RAG tier auto-selection → query analysis → tier recommendation, (10) Document chunking → LLM-suggested character-offset boundaries, (11) KG enrichment → relation inference between entities, (12) Procedural memory evolution → failure analysis + improvement suggestions, (13) Speaker intent → intent + urgency beyond emotion, (14) Home automation → natural language command interpretation, (15) Agent task decomposition → step breakdown with complexity estimates, (16) Conversation sentiment trend → trend summary (improving/stable/declining), (17) Multi-agent consensus → response synthesis from multiple agents.

## 244. LLM Response Caching & Progressive Enhancement

`CachedLlmEnhancer` wraps any `LlmEnhancer` with a FNV-1a hash-keyed response cache. Identical prompts within the TTL window are served from cache, eliminating redundant LLM calls. Progressive enhancement via `should_enhance()`: if the heuristic confidence >= `min_heuristic_confidence` (default 0.85), the LLM call is skipped entirely. `EnhancementCostTracker` records per-module call counts and estimated token usage. `batch_generate()` combines N prompts into a single LLM call with numbered JSON array response, falling back to individual calls on parse failure.
...
## 255. WASAPI Loopback Input (Virtual Speaker Capture)

The `ai_virtual_mic` input device dropdown lists physical microphones (🎤) AND every output device as a loopback source (🔊 `[loopback]`). On Windows, cpal's WASAPI backend transparently enables `AUDCLNT_STREAMFLAGS_LOOPBACK` when `build_input_stream()` is called on an output device, capturing whatever audio is playing on that speaker — system sounds, browser calls, other apps. No virtual audio driver is required. `DeviceInfo` carries a `DeviceKind` discriminator (Microphone/Loopback) so `start_audio()` picks the correct host list. A toggleable "🎚 Show levels" probe system spawns parallel WASAPI shared-mode streams, one per source, with smoothed RMS→dB and a 10-cell bar rendered next to each entry in the combo, so the user can instantly see which input is receiving the sound they want to hook into.

## 256. Audio Priority Protocol — Acoustic Floor Control

Multi-instance voice coordination via inaudible ultrasonic beacons (15.0–16.6 kHz). Multiple `ai_virtual_mic` clients in a shared voice call (Discord, in-game, Zoom) signal each other purely through the audio channel — no network link required between clients. Each client owns a **slot** (beacon frequency, 200 Hz apart) that it pulses as IDLE (30 ms every 2 s, −55 dBFS), ACTIVE (50 ms every 400 ms, −40 dBFS), or END (double-pulse). A reserved OVERRIDE frequency (16.6 kHz) lets authorized users force-interrupt lower-priority speakers. Detection uses the **Goertzel algorithm** (O(N) per frequency, no FFT dependency) with Hann windowing and neighbor-dominance filtering to prevent spectral leakage false positives.

Key components: `ToneEncoder` (phase-continuous sine generator), `GoertzelDetector` (narrow-band magnitude), `ToneDecoder` (multi-bin + noise-floor EMA + confirm-windows), `FloorMonitor` (per-slot active/connected state, timeout expiry, `can_speak(priority)`), `BeaconScheduler` (IDLE/ACTIVE/END/OVERRIDE timing), `MessageRecorder` (VAD-segmented with configurable silence timeout 0.5–5 s), `MessageQueue` (FIFO with oldest-eviction), `MessagePlayer` (Hard/Soft/Finish interrupt policies + Restart/Continue/Continue5s resume strategies with sample-level fade envelope), `PriorityTable` with factory presets (Flat/Squad/Meeting). 58 tests.

## 257. Group Queue Runtime and Host Network
...
## 269. Anti-Hallucination Pipeline: Strategy-Based Claim Processing

An anti-hallucination pipeline that processes every LLM response through claim decomposition and groundedness verification. The core idea: instead of treating hallucination as binary (yes/no), the system offers **7 strategies** for handling ungrounded claims: **Omit** (silently remove), **Mark** (inline `[unverified]` tag), **Warn** (log warning but keep), **Footnote** (numbered reference to verification status), **VerifyThenMark** (attempt RAG/web verification first, then mark if still ungrounded), **VerifyThenOmit** (verify first, remove if ungrounded), **Ask** (pause and request human confirmation). The strategy is configurable per deployment — a chatbot might use Mark, while a medical assistant might use VerifyThenOmit. The pipeline reuses `extract_claims()` from `hallucination_detection.rs` as the single source of claim decomposition, avoiding duplication across modules.

## 270. Calibrated Abstention: When Not Answering Is the Right Answer

When a model's confidence in its response falls below a threshold (default 0.3), the system abstains — returning a polite "I don't know" instead of a potentially false answer. This is implemented as `AbstentionGuard` in the guardrail pipeline (stage: PostReceive). Unlike simple confidence filtering, calibrated abstention integrates with per-claim scoring: if the overall response has high confidence but one critical claim does not, the system can still flag or abstain on that specific claim. The threshold is tuned per RAG tier — Enhanced uses 0.4 (conservative), Thorough uses 0.3, Agentic uses 0.25 (more aggressive, since it has more verification tools available). Auto-temperature automatically forces low temperature (0.3) for detected factual queries, reducing hallucination at the source.

## 271. Faithfulness Scoring: Measuring Factual Alignment

Faithfulness measures how well a generated response aligns with its source context. The `FaithfulnessScorer` decomposes a response into atomic claims (one fact per claim), then evaluates each against the provided context using Natural Language Inference (NLI). Two NLI methods are available: **WordOverlap** (Jaccard similarity between claim tokens and context tokens — zero cost, lower precision) and **LlmNli** (an LLM prompt asks "Does the context entail this claim?" — higher cost, better precision). Each claim receives a verdict: **Entailed** (supported by context), **Contradicted** (conflicts with context), or **Neutral** (neither supported nor contradicted). The overall faithfulness score is the ratio of entailed claims. Grounded generation extends this by anchoring each sentence to specific source chunks — if a sentence cannot be anchored to any chunk with similarity above a threshold (default 0.5), it is marked as ungrounded.

## 272. Chain-of-Verification (CoVe): Self-Correcting Responses

Chain-of-Verification is a multi-step pipeline where the system verifies its own output before presenting it to the user. Steps: (1) Generate an initial response normally. (2) Extract individual claims from the response. (3) For each claim, generate a verification question (e.g., claim "Paris is the capital of France" → question "What is the capital of France?"). (4) Answer each verification question independently using RAG or web search — critically, without access to the original response, to avoid confirmation bias. (5) Compare the independent answers with the original claims. (6) If contradictions are found, correct the response. Three correction modes control how corrections appear: **Replace** (silently rewrite), **Annotate** (add inline `[corrected: ...]` notes), **Footnote** (numbered corrections at the end). A hard cap on `max_claims_to_verify` (default 10) prevents cost explosion, and the total LLM calls are integrated with the RAG tier's `max_extra_llm_calls` budget.

## 273. Quality Gates: Configurable Quality SLAs

Quality gates are configurable thresholds on response quality metrics that determine whether a response is "good enough" to serve. Each gate specifies a **metric** (Faithfulness, Confidence, GroundingRatio, ConsistencyScore, CitationCoverage), a **threshold** (0.0-1.0), and an **action** (Fail — reject the response, Warn — serve but log, Log — record silently). `QualityGateRunner::production_defaults()` provides sensible starting gates. Gates integrate with the REST API (`POST /api/v1/verify/quality-check`), MCP protocol (`run_quality_gates` tool), and the `ai_proxy` gateway (per-API-key enforcement). This lets teams define and enforce quality SLAs: "no response with faithfulness < 0.7 leaves the system" is a single config line.

## 274. Academic Search: Unified Multi-Provider Paper Discovery

The `AcademicSearchProvider` trait unifies access to three academic databases: **arXiv** (open-access preprints, Atom/XML API), **Semantic Scholar** (citation graph + abstracts, REST/JSON), and **PubMed** (biomedical literature, E-utilities XML). Each provider has its own rate limiting (arXiv: 3s between requests, Semantic Scholar: 100 req/5min free, PubMed: 3 req/s without API key). Results are normalized into `AcademicPaper` structs with fields for title, authors, abstract, DOI, URL, citation count, fields of study, and external IDs. The `AcademicSearchAdapter` bridges to the standard `SearchProvider` trait so academic results flow through the same pipeline as web search results — enabling fact-checking claims against peer-reviewed literature.

## 275. Literature Review Pipeline: From Query to Formatted Review

The `LiteratureReviewPipeline` automates the traditionally manual process of literature review. Given a research topic, it: (1) searches across configured academic providers with configurable depth (Quick: 1 provider, Standard: 2, Deep: all 3), (2) filters and ranks papers by relevance, (3) synthesizes found papers into a structured review. Four synthesis styles are supported: **Narrative** (flowing prose organizing papers by theme), **Systematic** (structured methodology + inclusion criteria), **Annotated** (per-paper summary with critical commentary), **Comparative** (side-by-side comparison matrix). The review includes a bibliography in the chosen format (BibTeX, APA, MLA, Chicago, IEEE). The pipeline integrates `FaithfulnessScorer` to verify that claims in the synthesized review are actually supported by the source papers — ensuring the review itself doesn't hallucinate.

## 276. Composable Prompt Fragments: System Prompts You Assemble Like Lego

Traditional system prompts are a single monolithic string. `PromptBuilder` flips the model: build a prompt from **fragments** — small, named, categorised chunks (safety rules, tool guidance, style nudges, platform hints) that compose deterministically. Each `PromptFragment` carries a `priority` (lower = earlier in the final prompt), a `FragmentCategory` (`Safety`, `ToolGuidance`, `Context`, `Style`, `ModeSpecific`, `PlatformSpecific`, `Custom`), and optional `Platform` gating (`ClaudeCode`, `Gui`, `Server`, `Any`). The `PromptContext` describes the runtime ("autonomous agent on a server, no GUI"); the builder filters and sorts applicable fragments, then concatenates them into a final prompt plus an `AppliedFragments` manifest showing exactly what went in. The crate ships **11 catalog fragments** and **6 presets** (`Minimal`, `ToolUseChatbot`, `RagAssistant`, `AgenticLoop`, `ResearchAgent`, `CodeDeveloper`). Priority ranges are conventionalised: 0-9 Safety, 10-19 ToolGuidance, 20-29 Context, 30-39 Style, 40-49 ModeSpecific, 50-59 PlatformSpecific, 100+ Custom. Behind feature `prompt-fragments`; integrates with `Butler::recommend_prompt_fragments()` which maps keyword intents (e.g., "search papers", "write code") to a suggested preset + extras.

## 277. Claude Code Permission-Label Adapter: Internal Triples → Host Vocabulary

Internally, the crate represents tool permissions as a `PermissionRequirement` triple (risk level + default decision + scope hint) and a `DefaultDecision` enum (`Allow`, `Prompt`, `Deny`). Claude Code, by contrast, exposes 5 fixed permission labels: `ReadOnly`, `WorkspaceWrite`, `DangerFullAccess`, `Prompt`, `Allow`. `PermissionRequirement::to_claude_code_label()` performs the mapping deterministically so a crate caller hosted inside Claude Code gets the right UX affordance without hand-maintaining a second enum. The mapping is table-driven — adding a new `DefaultDecision` variant requires updating exactly one match; the compiler catches anything missed. Behind feature `autonomous`.

## 278. In-Crate Stall Detection: Heuristics Over LLM Calls

A **stall** is the state where the agent is burning budget without user-visible progress. Rather than asking "is this stuck?" with an LLM (expensive, slow, circular), `stall_detection` ships cheap deterministic heuristics. Signals: **`RepeatedToolCall`** (three or more identical `(tool_name, args)` FNV-1a hashes in the last 8 invocations — hard invariant, never a false positive for productive work), **`Frustrated`** (latest user message classifies as `EmotionCategory::Frustrated` via `KeywordEmotionDetector`), and (V95) **`Overheating`** (too many distinct tool calls within a sliding time window — catches "runaway but non-identical" loops). `StallHeuristic` trait has four methods (`observe_tool_call`, `observe_user_message`, `check`, `reset`), making the heuristic pluggable. Privacy invariant: only **derived signals** are stored — a `u64` hash per tool call and an `Option<EmotionCategory>` for the last message. Raw text never persists. Precedence when signals collide: `RepeatedToolCall` > `Overheating` > `Frustrated` (strongest invariant wins). `LoopStatus::UserStalled` + `AggregatedMetrics::user_stall_events_total` + OTel span `agent.user_stall_detected` wire the signal into the rest of the stack. Behind feature `stall-detection`.

## 279. LLM-Assisted Stall Detection: Second Opinion on a Cooldown

When keyword heuristics are ambiguous, a caller can wrap any `StallHeuristic` in `LlmAssistedStallDetector`, which calls a caller-provided `Fn(&LlmVerdictInput) -> LlmVerdict` on a configurable cooldown (default 30s, anchored to `observe_user_message` so it's naturally rate-limited to "once per user turn at most"). The LLM sees only `recent_tool_names` (capped at 16) and `last_user_message` — no full transcript. Inner (cheap) heuristic always wins when it fires; the LLM verdict is consulted only when the inner says `Continue`. Verdicts are `Stalled(StallSignal)`, `Continue`, or `Abstain`. Multi-language lexicons (English / Spanish / French / German) handle the `Frustrated` signal across the most common caller locales. Behind feature `stall-detection-llm`.

## 280. Ephemeral Sub-Agents: Fork / Teammate / Explore

Orthogonal to the persistent `AgentOrchestrator`, sub-agents are **fire-and-forget delegations**: the main agent hands off a bounded sub-task, gets back a single consolidated `SubAgentResult`, and the sub-agent is discarded. Three `SubAgentKind` patterns mirror the Claude Code `Task` tool: **`Fork`** (a copy of the main agent runs a parallel branch), **`Teammate`** (a differently-configured agent handles a specialised slice), **`Explore`** (a scoped research/search sub-task). `IsolationLevel` describes environment separation: `InProcess`, `ContextIsolated`, `ExternalProcess` (worktree/subprocess — routed to the caller). The crate ships a default **LLM-free** `InProcessSubAgentRunner` that echoes the spec back as a `Completed` result; callers plug in LLM-backed runners on top. Runners return `Deferred` (not `Failed`) when they can't handle a spec, letting callers chain runners (try default, fall back to worktree runner on `Deferred`). Behind feature `sub-agents`.

## 281. AgenticLoop Auto-Integration: Stall Detection Without Plumbing

`AgenticLoop` owns an optional `Box<dyn StallHeuristic>` gated on `feature = "stall-detection"`. Callers attach a heuristic via `AgenticLoop::new(config).with_stall_heuristic(Box::new(KeywordStallDetector::new()))`. During `process()`, the user message is forwarded to `observe_user_message` before the first iteration (also the LLM cooldown anchor), and after each iteration new `ToolCall`s are hashed via `hash_tool_call` and fed through `observe_tool_call`. A `Stalled` verdict sets `state.status = LoopStatus::UserStalled` and breaks the main loop. `stall_heuristic()` / `stall_heuristic_mut()` accessors let callers inspect post-loop state (e.g., `last_emotion`). When the feature is disabled, field and accessors vanish — zero runtime cost, zero API surface.

## 282. `#[non_exhaustive]` Signal Enums: Growing Without Breaking

Both `StallSignal` and `SubAgentKind` are marked `#[non_exhaustive]`. Callers matching on them must include a `_` arm. This costs one line per match site today but pays for itself later: adding signals like `TokenBurnExhausted`, `BudgetSpike`, or new sub-agent kinds like `Reviewer` / `Validator` becomes a minor bump (additive) instead of a major (breaking) bump. Telemetry accepts signal names as `&str` rather than the typed enum for the same reason — the telemetry module compiles in every feature configuration, so decoupling it from the feature-gated enum keeps counter code callable regardless of which signals were compiled in.

## 283. `ErrorCode` Trait: Stable Codes, Structured Fields, Localized Templates

Free-text `Display` strings ("API error 429: rate limited") force oncall and dashboards to regex-parse messages to slice failures by provider × kind. The `error_taxonomy::ErrorCode` trait fixes that with three obligations per error type: `code() -> &'static str` (a stable machine identifier like `PROVIDER_RATE_LIMITED`), `fields() -> Vec<(&'static str, String)>` (the variable parts, e.g. `("provider", "anthropic")`, `("retry_after_ms", "2000")`), and an optional `category()` for coarse taxonomies. The catalog is split across **`errors/en.json`** and **`errors/es.json`** (83 entries each at V116); each value is a template with `{field}` placeholders. The runtime substitutes fields into the template at format time, so the same error renders as `"Anthropic API error 429 (rate_limit_error): too many requests"` in English or `"Error de API de Anthropic 429 (rate_limit_error): demasiadas peticiones"` in Spanish without any code change.

## 284. `StructuredError`: One Shape for Logs, Traces, and i18n

`StructuredError::from_err(&dyn ErrorCode)` collapses any `ErrorCode` impl into a serializable shape `{ code, fields, category? }` that is the same regardless of which underlying error type produced it. Loggers can ship it as JSON; OTel spans can map `code → error.code` and each `(k, v)` field → `error.fields.<k>` (V118 wiring); the i18n resolver (`localize(locale, code, fields)`) renders it into a user-visible string in any supported locale. The shape is the contract — callers depend on `code` and field keys, not on `Display`.

## 285. Additive Migration to `ErrorCode` Without Breaking Existing Tests

A clean `thiserror` rewrite of every error type would have broken the 22 tests that already assert against the inherent `code()` method (which returns coarse strings like `"PROVIDER"` or `"CONFIG"` for API stability). The migration chosen for V113-V116 is **additive**: the existing `Display`/`Error` impls and inherent `code()` stay untouched; an `impl ErrorCode for X` block is added alongside, providing fine-grained codes via the trait. Callers writing `err.code()` keep getting the coarse inherent code (existing behavior preserved); callers needing fine-grained codes write `<X as ErrorCode>::code(&err)`. As of V116, 16 error types implement `ErrorCode` (`AiError` umbrella + 8 sub-types in `error.rs`, `RagPipelineError`, `EmbeddingError`, `KpkgError`, `AnthropicAdapterError`, `OpenAIAdapterError`, `HfError`, `ResilientError`). `ResilientError::AllProvidersFailed` decomposes its inner `(provider, message)` list into `attempted_count`, `providers` (joined), and `detail` so dashboards can branch on count or on a specific failing provider without parsing the aggregated string.

## 286. `StructuredError` → OTel Span Attributes (V118)

When an `AiSpan` records an error, the runtime calls `StructuredError::from_err(&err)` on any `ErrorCode` impl and emits structured OTel attributes: `error.code = "PROVIDER_RATE_LIMITED"`, `error.category = "Provider"`, plus one `error.fields.<k>` attribute per `(k, v)` field pair. Generic `&dyn std::error::Error` paths fall back to `error.message = err.to_string()`. The mapping is uniform across every span emitter — provider calls, RAG queries, multi-agent steps, autonomous iterations. Because the codes are stable across locale (`PROVIDER_RATE_LIMITED` is the same key the i18n resolver uses to render English or Spanish), span attributes line up with log fields and user-visible messages without translation drift. Dashboards can segment by failing provider × error code × retry-after bucket without regex-parsing message strings.

## 287. Stuck Detector: Heuristic Loop-Pathology Guard with Critique Recovery (V119)

A self-contained `StuckDetector` (`autonomous_loop` module, feature `autonomous`) observes each iteration's output + tool-call sequence and emits an optional `StuckSignal` from four heuristics: **OutputRepetition** (Jaccard similarity over recent outputs above threshold), **ActionLoop** (same tool-call hash repeated in a sliding window), **RetryWithoutChange** (a tool result reuses the same args after failure), **NoProgress** (N iterations with no new tool calls or terminal output). Three presets — `default()` (sim=0.85), `aggressive()` (sim=0.7), `permissive()` (sim=0.95) — let callers tune sensitivity without touching the runtime. Paired with `CritiqueRefiner`: instead of resetting on stuck-detection, the runtime constructs a meta-prompt asking the same model to diagnose its loop and propose a corrective action, then resumes — turning stuck-detection into a *recovery* signal, not a *kill* signal.

## 288. Stuck Detector Wired into Two Runtime Shapes Unchanged (V120-V121)

The same `StuckDetector` mixin attaches byte-for-byte unchanged into two structurally different runtimes. **V120**: the autonomous agent's iteration loop calls `observe()` after each tool-call result. **V121**: the multi-agent `PatternRunner` (round-robin / debate / nested-chat patterns) calls it after each `AgentTurn`, with per-agent output history so an agent repeating *itself* trips the detector even when the conversation alternates with another agent. Both wirings are opt-in via builder method (`.stuck_detector(...)`) and preserve previous behaviour byte-for-byte when not used. The `permissive()` preset exists because RoundRobin × 2 agents × 2 rounds can produce template-string traffic with high enough Jaccard to trip the 0.85 default — that lesson is encoded in the preset naming so callers using template-heavy patterns reach for `permissive()` first.

## 289. Parallel Read-Only Tool Execution via `std::thread::scope` (V122)

When an autonomous iteration parses ≥2 tool calls and *every* call is name-matched against an allow-list of side-effect-free tools (`read_file`, `list_files`, `glob`, `web_search`, `vector_search`, `rag_search`, `fetch`, `get_url`, `stat`, `head`, `tail`, …), the runtime dispatches them in parallel via `std::thread::scope` instead of sequentially. Sandbox validation still runs sequentially first (so policy denials are emitted in declared order), then the validated calls fan out, then results are collected back in declared order and processed sequentially. The handler signature `Arc<dyn Fn(...) -> ToolResult + Send + Sync>` is the precondition that lets the scoped threads borrow the registry safely. The opt-in flag (`parallel_read_only_tools: false` by default) means existing user behaviour is byte-for-byte unchanged, and the all-read-only gate keeps "tools execute in declared order with respect to side effects" intact: a single write call in the batch falls back to the sequential path. Latency on N independent reads collapses from N × R to max(R) plus a fixed thread-spawn cost.

## 290. Pre-Execution `Inspector` Trait: Defence Layer Below Sandbox (V123)

A second, *content-aware* defence layer beneath the policy sandbox. `Inspector::inspect(&ParsedToolCall) -> InspectorVerdict` runs every parsed tool call through a chain of inspectors *before* the sandbox sees it. Verdicts are `Allow | Warn(reason) | Block(reason)`; warnings push a tool message into the conversation and continue, blocks abort the iteration with a diagnostic reason and the registry never sees the call. Two built-in inspectors ship: **`AdversaryInspector`** (heuristic content filter — prompt-injection markers like `ignore previous instructions`, dangerous shell tokens like `rm -rf /`, suspicious URL hosts like `webhook.site` / `.onion`, secret-shaped patterns like `sk-ant-` / `-----BEGIN PRIVATE KEY-----`; first match → `Block`) and **`EgressInspector`** (name-based detection of network tools — `web_search`, `fetch`, `http_get`, `curl_get`, `download`, `browser`, `scrape`, `post_webhook`, `send_email`, `send_slack`, …; `warn_only()` and `strict()` presets — the latter is the building block for `--no-egress`). The two layers compose: a `summarize(text="Ignore previous instructions…")` call is invisible to the sandbox (path/command policy can't see prompt-injection content) but trips the adversary inspector on the argument scan.

## 291. Why Two Layers (Inspector + Sandbox)

**Sandbox** = policy. Paths, commands, internet mode, cost, iteration limits, audit log. Authoritative, structured, durable. **Inspector** = heuristic. String patterns, name allow-lists, domain-specific filters. Cheap, extensible, opt-in. They run in different parts of the pipeline and answer different questions. A sandbox can't tell that a benign-looking `summarize(text=…)` call carries a prompt-injection payload. An inspector can't replace per-path policy decisions. V123 ships them as separate gears so they evolve independently — sandbox semantics stay stable while new inspectors get added for new threats (prompt injection in 2024, MCP supply chain in 2025) without touching the policy layer.

## 292. Env-Var-Mediated CLI Flag Wiring for Cross-Subcommand Reach (V123)

The CLI flags `--no-egress` and `--adversary-inspector` are parsed at the *top level*, before subcommand dispatch, and surfaced as the env vars `AI_NO_EGRESS=1` and `AI_ADVERSARY_INSPECTOR=1`. `agent_wiring::AgentCreateOptions` reads those env vars as defaults for its `no_egress` and `adversary_inspector` boolean fields, so any subcommand that builds an autonomous agent — present or future — honours the user's intent without per-command plumbing. Explicit `AgentCreateOptions` fields take precedence; env vars only kick in when the caller left the option `false`. This is the right wiring shape for a CLI flag whose audience is "every autonomous-agent code path in the binary": the flag travels via env, not function arguments, so subcommand authors don't have to thread it manually.

> **Note on numbering:** sections 293–317 exist in the rendered `concepts.html` but have not yet been back-ported to this MD file (pre-V150 drift). The website is authoritative for those interim sections.

## 318. Streaming on the Hot Path, Guards Stay Honest (V150)

V78's `ai_proxy` did `resp.bytes().await` on every forwarded response. JSON was fine; SSE was broken — clients asking for `stream:true` got the entire token stream at the very end. V150 swaps in `reqwest::Response::bytes_stream()` + `axum::body::Body::from_stream()` on the three forwarding sites that need it: the gateway passthrough fallback, the chat handler's stream branch, and the free path. The non-stream chat path keeps the buffered behavior — output guards (PII, toxicity, faithfulness scoring) cannot scan partial chunks — but when an upstream comes back SSE-shaped on that path, the response now carries `x-streaming-disabled: output-guard-active` so clients can tell "no stream available" apart from "stream silently bufferized." The choice not to fold the two paths into one shared helper was deliberate: `forward_core` stays buffered, `forward_core_streamable` handles the stream-or-buffer dispatch on content-type. ~130 LoC of duplication beats a shared helper that would have leaked stream semantics into the guard path. **Per-chunk inactivity timeout**: every chunk arrival is wrapped in `tokio::time::timeout(stream_chunk_timeout, ...)`. Default 30s, configurable via `[routing] stream_chunk_timeout_secs`. A slow-loris backend that emits one byte then stalls is closed inside the configured window; the metric `proxy_stream_aborts_chunk_timeout` makes the abort visible. Five Prometheus counters total: chunks total, aborts (chunk timeout / upstream / client close), and guard-disabled count. Streams are not cacheable — the cache layer was already bypassed for `stream:true` requests in V78. The combination — SSE passthrough + per-chunk timeout + explicit guard-disable header + flat Prometheus counters — is what makes streaming federation deploy-grade in a single patch.
