...
| `agentic_edit` | the model gets an EXISTING crate (four modules plus a passing test suite) and a change request that states the symptom, never the file. Two gates, scored apart: the seeded tests must still pass, and the requested change must be there. | 6 |
