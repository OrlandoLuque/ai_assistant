...
pub use scoring::{accuracy, mean_score, pass_at_k, DefaultScorer, EloCalculator, ProblemScorer};

pub use runner::{
    BenchmarkRunResult, BenchmarkSuiteRunner, ModelIdentifier, ProblemResult, RunConfig, TokenUsage,
};

pub use comparison::{ComparisonConfig, ComparisonMatrix};

pub use ablation::{
    AblationEngine, AblationRecommendation, AblationResult, AblationStudy, RunSummary,
};
...
pub use agent_config::{ConfigMeasurement, EvalAgentConfig, MultiModelGenerator, SearchDimension};
...
pub fn register_eval_tools(
    ...
    server.register_tool(
        McpTool::new(
            "eval.create_dataset",
            "Create an inline benchmark dataset with custom problems",
        )
        .with_property("name", "string", "Dataset name", true)
        .with_property(
            "suite_type",
            "string",
            "Suite type: humaneval, mmlu, gsm8k, custom, etc.",
            false,
        )
        .with_property(
            "num_problems",
            "number",
            "Number of sample multiple-choice problems to generate (default 5)",
            false,
        ),
        move |args| {
            let name = args
    ...
    server.register_tool(
        McpTool::new(
            "eval.run",
            "Run a benchmark evaluation against the configured LLM",
        )
        .with_property(
            "dataset",
            "string",
            "Dataset name (must be created first with eval.create_dataset)",
            true,
        )
        .with_property(
            "model_name",
            "string",
            "Model name for tracking (default: 'default')",
            false,
        )
        .with_property(
            "provider",
            "string",
            "Provider name for tracking (default: 'local')",
            false,
        )
        .with_property(
            "samples_per_problem",
            "number",
            "Samples per problem for Pass@k (default 1)",
            false,
        )
        .with_property(
            "temperature",
        ...
        move |args| {
            ...
            let dataset = datasets.get(dataset_name).ok_or_else(|| {
                format!(
                    "Dataset '{}' not found. Create it first with eval.create_dataset",
                    dataset_name
                )
            })?;
    ...
    server.register_tool(
        ...
        .with_property(
            "run_ids",
            "string",
            "Comma-separated run IDs to compare",
            true,
        ),
        move |args| {
            let run_ids_str = args
                .get("run_ids")
                .and_then(|v| v.as_str())
            ...
            let best_json: serde_json::Value = best
                .iter()
    ...
    server.register_tool(
        ...
        move |args| {
            ...
            if ids.len() >= 2 {
                let runs: Vec<BenchmarkRunResult> = ids
                    .iter()
...
mod tests {
    ...
    fn test_eval_mcp_get_result() {
        ...
        call_eval_tool(
            &server,
            "eval.create_dataset",
            serde_json::json!({"name": "ds-get", "num_problems": 2}),
        );
        let run_result = call_eval_tool(
            &server,
            "eval.run",
            serde_json::json!({"dataset": "ds-get", "model_name": "m1"}),
        );
    ...
    fn test_eval_mcp_run_not_found() {
        ...
        let result = call_eval_tool(
            &server,
            "eval.get_result",
            serde_json::json!({"run_id": "nonexistent-id"}),
        );
    ...
    fn test_eval_mcp_compare_runs() {
        ...
        call_eval_tool(
            &server,
            "eval.create_dataset",
            serde_json::json!({"name": "ds-cmp", "num_problems": 3}),
        );
        let r1 = extract_eval_text(&call_eval_tool(
            &server,
            "eval.run",
            serde_json::json!({"dataset": "ds-cmp", "model_name": "model-a", "provider": "p1"}),
        ));
        let r2 = extract_eval_text(&call_eval_tool(
            &server,
            "eval.run",
            serde_json::json!({"dataset": "ds-cmp", "model_name": "model-b", "provider": "p2"}),
        ));
    ...
    fn test_eval_mcp_report() {
        ...
        call_eval_tool(
            &server,
            "eval.create_dataset",
            serde_json::json!({"name": "ds-rpt", "num_problems": 2}),
        );
        let r1 = extract_eval_text(&call_eval_tool(
            &server,
            "eval.run",
            serde_json::json!({"dataset": "ds-rpt", "model_name": "m1"}),
        ));
    ...
    fn test_eval_mcp_run_with_config_change() {
        ...
        call_eval_tool(
            &server,
            "eval.create_dataset",
            serde_json::json!({"name": "ds-cfg", "num_problems": 2}),
        );
        ...
        let det1 = extract_eval_text(&call_eval_tool(
            &server,
            "eval.get_result",
            serde_json::json!({"run_id": id1}),
        ));
        let det2 = extract_eval_text(&call_eval_tool(
            &server,
            "eval.get_result",
            serde_json::json!({"run_id": id2}),
        ));
