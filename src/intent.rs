...
impl IntentClassifier {
    pub fn new() -> Self {
        ...
        patterns.insert(Intent::Question, vec![
            ...
            "is it", "are there", "can you", "could you", "do you", "does it",
            "?",
        ]);

        patterns.insert(Intent::Command, vec![
        ...
        patterns.insert(Intent::Thanks, vec![
            "thank", "thanks", "appreciate", "grateful", "cheers",
        ]);

        patterns.insert(Intent::Complaint, vec![
            "doesn't work", "not working", "broken", "bug", "error",
            "wrong", "incorrect", "bad", "terrible", "awful",
        ]);

        patterns.insert(Intent::Request, vec![
            "please", "could you", "would you", "can you", "i need",
            "i want", "i'd like", "help me",
        ]);

        patterns.insert(Intent::Clarification, vec![
            "what do you mean", "i don't understand", "can you explain",
            "be more specific", "elaborate", "clarify",
        ]);

        patterns.insert(Intent::Confirmation, vec![
            "yes", "yeah", "yep", "correct", "right", "exactly",
            "sure", "ok", "okay", "agreed", "affirmative",
        ]);

        patterns.insert(Intent::Negation, vec![
            "no", "nope", "not", "never", "wrong", "incorrect",
            "disagree", "negative",
        ]);

        patterns.insert(Intent::CodeRequest, vec![
            "code", "function", "class", "implement", "programming",
            "script", "algorithm", "syntax", "debug",
        ]);

        patterns.insert(Intent::Explanation, vec![
            "explain", "tell me about", "describe", "what is",
            "definition", "meaning of",
        ]);

        patterns.insert(Intent::Comparison, vec![
            "compare", "difference", "versus", "vs", "better than",
            "worse than", "similar", "different",
        ]);
