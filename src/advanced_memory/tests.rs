...
fn test_new_episode_importance_clamped() {
    ...
    assert!(
        (ep.importance - 1.0).abs() < 1e-6,
        "Importance should clamp to 1.0"
    );

    let ep2 = new_episode("x", "c", -0.5, vec![], vec![]);
    assert!(
        (ep2.importance - 0.0).abs() < 1e-6,
        "Importance should clamp to 0.0"
    );
}
...
fn test_episodic_store_recall_by_tags() {
    ...
    let results = store.recall_by_tags(&["rust".to_string(), "ml".to_string()], 10);
    // e3 matches 2 tags, e1 matches 1, e2 matches 1
    assert!(!results.is_empty());
    assert_eq!(
        results[0].id, "e3",
        "Episode with most matching tags should be first"
    );
}
...
fn test_episodic_store_temporal_decay() {
    ...
    assert_eq!(
        results[0].id, "new",
        "Recent episode should rank higher with decay"
    );
}
...
fn test_procedural_find_by_condition() {
    ...
    assert_eq!(
        results[0].id, "p1",
        "Best matching procedure should be first"
    );
}
...
fn test_entity_duplicate_detection() {
    let mut store = EntityStore::new();
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("first add ok");
...
fn test_entity_find_by_name() {
    let mut store = EntityStore::new();
    store
        .add(make_entity("e1", "Python", "language"))
        .expect("add Python entity for find_by_name");
    store
        .add(make_entity("e2", "Rust", "language"))
        .expect("add Rust entity for find_by_name");
...
fn test_entity_get() {
    ...
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("add entity for entity_get");
...
fn test_entity_update_attributes() {
    ...
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("add entity for update_attributes");
    ...
    assert_eq!(
        ent.attributes.get("version"),
        Some(&serde_json::json!("1.77"))
    );
}
...
fn test_entity_add_relation() {
    ...
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("add Rust for add_relation");
    store
        .add(make_entity("e2", "Cargo", "tool"))
        .expect("add Cargo for add_relation");
    ...
    store
        .add_relation("e1", rel)
        .expect("add uses-relation from Rust to Cargo");
...
fn test_entity_merge() {
    ...
    assert_eq!(
        store.len(),
        1,
        "Source entity should be removed after merge"
    );
    ...
    assert!(
        store.find_by_name("Rust Programming").is_none(),
        "Source name index should be removed"
    );
}
...
fn test_entity_merge_self() {
    ...
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("add entity for merge_self test");
...
fn test_entity_merge_target_not_found() {
    let mut store = EntityStore::new();
    store
        .add(make_entity("e2", "Python", "language"))
        .expect("add entity for merge_target_not_found");
...
fn test_entity_merge_source_not_found() {
    ...
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("add entity for merge_source_not_found");
...
fn test_entity_remove() {
    ...
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("add entity for entity_remove");
    assert_eq!(store.len(), 1);
...
fn test_entity_all() {
    ...
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("add Rust for entity_all");
    store
        .add(make_entity("e2", "Python", "language"))
        .expect("add Python for entity_all");
    let all = store.all();
    assert_eq!(all.len(), 2);
}
...
fn test_consolidation_no_overlap() {
    ...
    assert_eq!(
        ...
        "No clusters when similarity threshold is very high"
    );
}
...
fn test_consolidation_all_identical() {
    ...
    assert_eq!(
        result.clusters_found, 1,
        "All identical episodes should form one cluster"
    );
...
fn test_manager_find_entity() {
    let mut mgr = AdvancedMemoryManager::new();
    mgr.add_entity(make_entity("e1", "Rust", "language"))
        .expect("add entity via manager for find_entity");
...
fn test_manager_full_lifecycle() {
    ...
    // Add entity
    mgr.add_entity(make_entity("ent1", "Rust", "language"))
        .expect("add entity in manager_full_lifecycle");
...
fn test_entity_serialization() {
    ...
    e.attributes
        .insert("version".to_string(), serde_json::json!("2021"));
...
fn test_entity_store_to_from_json() {
    ...
    store
        .add(make_entity("e1", "Rust", "language"))
        .expect("add Rust for entity_store_to_from_json");
    store
        .add(make_entity("e2", "Python", "language"))
        .expect("add Python for entity_store_to_from_json");
...
fn test_episode_access_count_incremented_by_recall() {
    ...
    assert_eq!(
        ep.access_count, 1,
        "Access count should be incremented after recall"
    );
    ...
    assert_eq!(
        ep.access_count, 2,
        "Access count should be incremented again"
    );
}
...
fn test_entity_merge_preserves_earliest_first_seen() {
    ...
    store
        .add(e1)
        .expect("add target entity for first_seen test");

    let mut e2 = make_entity("e2", "Source", "type");
    e2.first_seen = 100; // earlier
    store
        .add(e2)
        .expect("add source entity for first_seen test");

    store
        .merge("e1", "e2")
        .expect("merge to preserve earliest first_seen");
...
fn test_entity_merge_accumulates_relations() {
    ...
    store
        .add(e1)
        .expect("add target entity for relations merge");
    ...
    store
        .add(e2)
        .expect("add source entity for relations merge");

    store
        .merge("e1", "e2")
        .expect("merge to accumulate relations");
    let merged = store
        .get("e1")
        .expect("merged entity with relations exists");
    assert_eq!(
        merged.relations.len(),
        2,
        "Relations from both entities should be present"
    );
}
...
fn test_pattern_extractor_default_patterns() {
    ...
    let episodes = vec![make_episode("e1", "Alice prefers Rust", &[], &[], 100)];
...
fn test_pattern_extractor_no_matches() {
    ...
    let relevant = facts
        .iter()
        .filter(|f| f.predicate != "is" || !f.subject.is_empty())
        .count();
...
fn test_fact_store_merge_existing() {
    ...
    assert!(
        ...
        "Duplicate should merge, returning false"
    );
...
fn test_memory_scheduler_mark_completed() {
    ...
    let job0_due = due
        .iter()
        .any(|j| matches!(j.task, SchedulerTask::Consolidate));
    assert!(!job0_due);
}
...
fn test_memory_extractor_extract_email() {
    ...
    let has_entity_update = results.iter().any(
        |r| matches!(r, MemoryExtraction::EntityUpdate { attribute, .. } if attribute == "email"),
    );
...
fn test_memory_extractor_extract_date_iso() {
    ...
    let has_date = results.iter().any(
        |r| matches!(r, MemoryExtraction::NewFact { fact } if fact.object.contains("2026-03-15")),
    );
...
fn test_episodic_store_save_preserves_content() {
    ...
    store.add(make_episode(
        "e1",
        "special content with unicode: \u{00e9}\u{00e8}\u{00ea}",
        &["tag1", "tag2"],
        &[0.5, 0.5, 0.5],
        5000,
    ));
...
fn test_compressed_smaller_than_raw() {
    ...
    config
        .save_compressed("compressible", data.as_bytes())
        .unwrap();
    let files: Vec<_> = std::fs::read_dir(&dir)
        .unwrap()
        .filter_map(|e| e.ok())
...
fn test_load_compressed_nonexistent() {
    let result =
        AutoPersistenceConfig::load_compressed(std::path::Path::new("/nonexistent/file.json.gz"));
    assert!(result.is_err());
...
fn test_entity_query_by_name() {
    let mut store = EntityStore::new();
    store
        .add(make_entity("e1", "Alice Smith", "person"))
        .unwrap();
