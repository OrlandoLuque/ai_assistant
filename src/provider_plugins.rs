    ...
    pub fn check_health(&self) -> bool {
            ...
            .call()
        {
            Ok(resp) => {
                ...
                self.available.store(is_ok, std::sync::atomic::Ordering::SeqCst);
                is_ok
            }
            Err(_) => {
...
impl ProviderPlugin for OllamaProvider {
    ...
    fn generate_embeddings(&self, texts: &[&str]) -> Result<Vec<Vec<f32>>> {
        ...
        for text in texts {
            ...
            let body: Value = resp.into_json()?;
            if let Some(embedding) = body.get("embeddings")
...
impl ProviderPlugin for KoboldCppProvider {
    ...
    fn list_models(&self) -> Result<Vec<ModelInfo>> {
        ...
        let body: Value = resp.into_json()?;
        let model_name = body.get("result")
            .and_then(|r| r.as_str())
            .unwrap_or("unknown");
        ...
        let models = body.get("data")
            ...
            .map(|arr| {
                arr.iter()
                    .filter_map(|m| {
                        let id = m.get("id")?.as_str()?.to_string();
                        Some(ModelInfo::new(id, AiProvider::OpenAICompatible { base_url: base_url.clone() }))
                    })
                    .collect()
            })
            .unwrap_or_default();
...
pub fn discover_providers(config: &DiscoveryConfig) -> Vec<Box<dyn ProviderPlugin>> {
    ...
    if config.check_ollama {
        let provider = OllamaProvider::new(&config.ollama_url)
    ...
    if config.check_lm_studio {
        let provider = LmStudioProvider::new(&config.lm_studio_url)
            .with_timeout(config.timeout);
        if provider.check_health() {
            providers.push(Box::new(provider));
        }
    ...
    if config.check_text_gen_webui {
        let provider = TextGenWebUIProvider::new(&config.text_gen_webui_url)
            .with_timeout(config.timeout);
        if provider.check_health() {
            providers.push(Box::new(provider));
        }
    ...
    if config.check_kobold_cpp {
        let provider = KoboldCppProvider::new(&config.kobold_cpp_url)
            .with_timeout(config.timeout);
        if provider.check_health() {
            providers.push(Box::new(provider));
        }
