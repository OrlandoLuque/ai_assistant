...
fn test_generate_summary_with_content() {
    let messages = vec![
        (
            "user".to_string(),
            "The architecture uses microservices for deployment".to_string(),
        ),
        (
            "assistant".to_string(),
            "Microservices architecture provides scalability benefits".to_string(),
        ),
        (
            "user".to_string(),
            "We need more microservices testing coverage".to_string(),
        ),
    ];
...
fn test_generate_summary_decisions_and_questions() {
    let messages = vec![
        (
            "user".to_string(),
            "We decided to use Rust for the backend".to_string(),
        ),
        (
            "assistant".to_string(),
            "Agreed, Rust is a good choice".to_string(),
        ),
        (
            "user".to_string(),
            "What about the frontend framework?".to_string(),
        ),
        (
            "user".to_string(),
            "Have we chosen a database yet?".to_string(),
        ),
    ];
...
fn test_extract_highlights_conclusions() {
    let messages = vec![
        (
            "user".to_string(),
            "This is an important finding about performance".to_string(),
        ),
        (
            "assistant".to_string(),
            "A critical issue was identified in the pipeline".to_string(),
        ),
        (
            "assistant".to_string(),
            "In conclusion, we should refactor the module".to_string(),
        ),
        (
            "user".to_string(),
            "Therefore, the next step is clear".to_string(),
        ),
    ];
...
fn test_extract_highlights_action_items() {
    let messages = vec![
        (
            "user".to_string(),
            "We need to update the dependencies".to_string(),
        ),
        (
            "assistant".to_string(),
            "TODO: add error handling for edge cases".to_string(),
        ),
        ("user".to_string(), "The weather is nice today".to_string()),
        (
            "assistant".to_string(),
            "Action item: review the PR before merging".to_string(),
        ),
    ];
...
fn test_extract_context_entities() {
    let messages = vec![
        (
            "user".to_string(),
            "The system uses Rust and PostgreSQL for storage".to_string(),
        ),
        (
            "assistant".to_string(),
            "Indeed, Rust provides great performance with PostgreSQL".to_string(),
        ),
    ];
...
fn test_extract_beliefs() {
    let messages = vec![
        (
            "user".to_string(),
            "I think Rust is the best language for this project".to_string(),
        ),
        (
            "assistant".to_string(),
            "I believe the architecture is sound and well-designed".to_string(),
        ),
        (
            "user".to_string(),
            "We should add more comprehensive tests to the suite".to_string(),
        ),
...
fn test_repair_session_corrupted() {
    ...
    assert!(result
        .repair_notes
        .iter()
        .any(|n| n.contains("No messages could be recovered")));
}
...
fn test_session_context_serde() {
    let ctx = SessionContext {
        ...
        relations: vec![("Rust".to_string(), "is".to_string(), "fast".to_string())],
...
fn test_batch_executor_create_batch() {
    let executor = BatchExecutor::with_defaults();
    let requests = vec![JsonRpcRequest::new(serde_json::json!(1), "ping")];
...
fn test_static_completion_provider_add_values_and_complete() {
    ...
    provider.add_values(
        "lang".to_string(),
        vec!["Rust".into(), "Ruby".into(), "Python".into()],
    );
...
fn test_knowledge_tools_registration_without_graph() {
    ...
    let db_path = std::env::temp_dir().join(format!("test_kt_{}.db", uuid::Uuid::new_v4()));
...
fn test_knowledge_tools_search_invocation() {
    ...
    let db_path = std::env::temp_dir().join(format!("test_kt_search_{}.db", uuid::Uuid::new_v4()));
    {
    ...
    assert!(
        resp.result.is_some(),
        "search_knowledge should return result"
    );
...
fn test_knowledge_tools_annotations() {
    ...
    let db_path = std::env::temp_dir().join(format!("test_kt_ann_{}.db", uuid::Uuid::new_v4()));
    register_knowledge_tools(&mut server, db_path.clone(), None);
...
fn test_knowledge_tools_list_sources() {
    ...
    let db_path = std::env::temp_dir().join(format!("test_kt_list_{}.db", uuid::Uuid::new_v4()));
    {
        let db = RagDb::open(&db_path).expect("create db");
