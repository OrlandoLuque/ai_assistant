...
fn test_signature_compile_field_types() {
    let sig = Signature::new("typed", "Typed signature")
        .add_input(SignatureField::new("count", "a number").with_type(FieldType::Number))
        .add_output(SignatureField::new("result", "a boolean").with_type(FieldType::Boolean));
...
fn test_validate_inputs_success() {
    let sig = Signature::new("v", "validate").add_input(SignatureField::new("name", "a name"));
...
fn test_validate_inputs_optional_missing_ok() {
    let sig =
        Signature::new("v", "validate").add_input(SignatureField::new("name", "a name").optional());
    let inputs: HashMap<String, String> = HashMap::new();
    assert!(sig.validate_inputs(&inputs).is_ok());
}
...
fn test_f1_score_metric() {
    ...
    assert!(
        s > 0.0 && s < 1.0,
        "partial match should give 0 < score < 1, got {}",
        s
    );
...
fn test_contains_answer_metric() {
    ...
    assert_eq!(metric.score("The answer is 42 of course", "42"), 1.0);
...
fn make_training_examples() -> Vec<TrainingExample> {
    vec![
        ...
        TrainingExample {
            ...
            expected_outputs: HashMap::from([(
                "answer".to_string(),
                "No, Python is interpreted".to_string(),
            )]),
        },
        TrainingExample {
            inputs: HashMap::from([
            ...
            expected_outputs: HashMap::from([("answer".to_string(), "Blue".to_string())]),
        },
    ]
}
...
fn test_bootstrap_selects_best() {
    ...
    let result = optimizer
        ...
        .expect("bootstrap optimize in selects_best");
...
fn test_bootstrap_respects_budget() {
    ...
    let result = optimizer
        .optimize(&sig, &examples, &mut budget)
        .expect("bootstrap optimize in respects_budget");
...
fn test_grid_search_basic() {
    let variants = vec!["Be concise.".to_string(), "Think step by step.".to_string()];
...
fn test_grid_search_selects_best() {
    ...
    let result = optimizer
        .optimize(&sig, &examples, &mut budget)
        .expect("grid_search optimize in selects_best");
    let max_score = result
        .scores_history
        .iter()
        .cloned()
...
fn test_grid_search_respects_budget() {
    ...
    let result = optimizer
        .optimize(&sig, &examples, &mut budget)
        .expect("grid_search optimize in respects_budget");
    assert!(result.trials_run <= 2);
}
...
fn test_grid_search_no_examples() {
    ...
    let result = optimizer
        .optimize(&sig, &[], &mut budget)
        .expect("grid_search optimize with no examples");
...
fn test_random_search_respects_budget() {
    ...
    let result = optimizer
        .optimize(&sig, &examples, &mut budget)
        .expect("random_search optimize in respects_budget");
...
fn test_bayesian_optimizer_respects_budget() {
    ...
    let result = optimizer
        .optimize(&sig, &examples, &mut budget)
        .expect("bayesian optimize in respects_budget");
    assert!(result.trials_run <= 3);
}
...
fn test_bayesian_gp_predict_single_obs() {
    ...
    assert!(
        (mean - 1.0).abs() < 0.1,
        "mean at observed point should be near 1.0, got {}",
        mean
    );
    assert!(
        std_dev < 0.5,
        "std_dev at observed point should be small, got {}",
        std_dev
    );
}
...
fn test_self_reflector_generates_rules() {
    ...
    assert!(
        !rules.is_empty(),
        "reflector should generate at least one rule"
    );
}
...
fn test_self_reflector_no_instructions_rule() {
    ...
    let has_no_instructions = rules.iter().any(|r| r.condition == "no_instructions");
...
fn test_self_reflector_no_examples_rule() {
    ...
    let rules = reflector.reflect(&sig, &compiled, &[]);
    let has_no_examples = rules.iter().any(|r| r.condition == "no_examples");
...
fn test_self_reflector_few_examples_rule() {
    ...
    let rules = reflector.reflect(&sig, &compiled, &examples);
    let has_few_examples = rules.iter().any(|r| r.condition == "few_examples");
...
fn test_self_reflector_incomplete_input_coverage() {
    ...
    let incomplete = vec![TrainingExample {
        inputs: HashMap::from([("question".to_string(), "What?".to_string())]),
        expected_outputs: HashMap::from([("answer".to_string(), "Something".to_string())]),
    }];
...
fn test_self_reflector_incomplete_output_coverage() {
    ...
    assert!(
        has_coverage_rule,
        "should detect incomplete output coverage"
    );
}
...
fn test_self_reflector_typed_output_guidance() {
    ...
    let sig = Signature::new("typed", "Typed outputs")
        .add_output(SignatureField::new("count", "a count").with_type(FieldType::Number))
        .with_instructions("inst");
    let compiled = sig.compile();
...
fn test_self_reflector_apply_rules() {
    ...
    assert!(
        improved
            ...
            .as_deref()
            .unwrap_or("")
            .contains("Follow the field descriptions"),
...
fn test_end_to_end_optimize_and_reflect() {
    ...
    let opt_result = optimizer
        .optimize(&sig, &examples, &mut budget)
        .expect("optimize in end_to_end_optimize_and_reflect");
...
fn test_multiple_optimizers_comparison() {
    ...
    let r1 = bs
        .optimize(&sig, &examples, &mut budget1)
        .expect("bootstrap in optimizers_comparison");
    ...
    let r2 = gs
        .optimize(&sig, &examples, &mut budget2)
        .expect("grid_search in optimizers_comparison");

    // Random search
    let rs = RandomSearchOptimizer::new(3, Box::new(ContainsAnswer));
    let mut budget3 = EvaluationBudget::new(3, 5);
    let r3 = rs
        .optimize(&sig, &examples, &mut budget3)
        .expect("random_search in optimizers_comparison");
...
fn test_instruction_proposer_includes_field_names() {
    ...
    for c in &candidates {
        assert!(
            c.contains("context") || c.contains("question") || c.contains("answer"),
            "candidate should mention field names: {}",
            c
        );
    }
}
...
fn test_miprov2_selects_best() {
    ...
    let opt = optimizer
        .optimize(&sig, &examples, &metric, &mut budget)
        .expect("MIPROv2 optimize in selects_best");
    let max_score = opt
        .scores_history
        .iter()
        .cloned()
...
fn test_miprov2_respects_budget() {
    ...
    let opt = optimizer
        .optimize(&sig, &examples, &metric, &mut budget)
        .expect("MIPROv2 optimize in respects_budget");
...
fn test_contains_assertion_pass_case_insensitive() {
    ...
    assert_eq!(assertion.check("rust is great"), AssertionResult::Pass);
}
...
fn test_judge_metric_eval_metric_impl_score() {
    ...
    let judge = JudgeMetric::new(
        config,
        Box::new(
            |predicted: &str, expected: &str, _rubric: &JudgeRubric| -> f64 {
