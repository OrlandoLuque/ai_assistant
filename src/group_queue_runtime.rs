...
impl GroupQueueRuntime {
    pub fn new(config: RuntimeConfig, table: PriorityTable) -> Self {
        ...
        let player = MessagePlayer::new(
            config.interrupt_policy, config.resume_strategy, &config.protocol,
        );
        ...
        let jitter_offset_ms = (config.my_slot.as_u8() as u64 * 137) % config.protocol.jitter_max_ms.max(1);
        Self {
            config, table, scheduler, encoder, decoder, monitor,
            recorder, queue, player,
            status: RuntimeStatus::Disabled,
            ptt_held: false, override_held: false,
            started_at: Instant::now(), jitter_offset_ms,
    ...
    pub fn reconfigure(&mut self, new_config: RuntimeConfig, table: PriorityTable) {
        ...
        let silence_changed = new_config.protocol.silence_timeout_ms != self.config.protocol.silence_timeout_ms
        ...
        if player_changed || protocol_changed {
            self.player = MessagePlayer::new(
                new_config.interrupt_policy, new_config.resume_strategy, &new_config.protocol,
            );
        }
    ...
    pub fn set_ptt(&mut self, held: bool) {
        self.ptt_held = held;
        if self.config.capture_mode == CaptureMode::PushToTalk && !held && self.recorder.is_recording() {
            // Finalize on release
    ...
    pub fn set_override(&mut self, held: bool) {
        ...
        if self.override_held {
            ...
            self.status = RuntimeStatus::Transmitting { queued: self.queue.len() };
        } else {
            self.status = if self.queue.is_empty() { RuntimeStatus::Idle }
                else { RuntimeStatus::Waiting { queued: self.queue.len() } };
        }
    }

    /// Cancel the current message recording in progress.
    pub fn cancel_recording(&mut self) { self.recorder.cancel(); }
    ...
    pub fn peers_snapshot(&self) -> Vec<PeerSnapshot> {
        ...
        for slot_u8 in 0..self.config.protocol.slot_count {
            ...
            out.push(PeerSnapshot {
                ...
                display_name: assignment.map(|a| a.display_name.clone())
    ...
    pub fn process_mic(&mut self, samples: &[f32], is_voice: bool, now: Instant) -> Option<Vec<f32>> {
    ...
    pub fn tick_output(&mut self, output: &mut [f32], now: Instant) {
        ...
        let wait = self.config.protocol.jitter_max_ms.saturating_sub(self.jitter_offset_ms);
        ...
        for beacon in beacons {
            let samples_for_pulse = ((beacon.duration_ms as usize) * (self.config.protocol.sample_rate as usize)) / 1000;
            let n = samples_for_pulse.min(output.len());
            if n == 0 { continue; }
            let mut pulse_buf = vec![0.0f32; n];
            self.encoder.mix_pulse(&mut pulse_buf, beacon.frequency_hz, beacon.amplitude);
    ...
    fn advance_state(&mut self, _now: Instant) {
        if self.override_held { return; } // status is `Overriding`, nothing else to do
        ...
        if self.monitor.override_active() && !self.override_held {
            if self.player.is_playing() {
                self.player.request_interrupt();
                self.status = RuntimeStatus::Interrupted { queued: self.queue.len() };
        ...
        if !self.player.is_playing() {
            self.status = if self.recorder.is_recording() {
                ...
                RuntimeStatus::Waiting { queued: self.queue.len() }
            } else {
                RuntimeStatus::Idle
            };
        }
...
mod tests {
    ...
    fn runtime_vad_creates_messages() {
        ...
        assert!(rt.queue_len() >= 1, "queue should have a message after VAD finalize");
    }
    ...
    fn runtime_override_held_bypasses_queue() {
        ...
        table.assign(SlotAssignment {
            slot: rt_cfg_my_slot, priority: Priority(10),
            can_override: true, display_name: "Leader".into(),
        });
    ...
    fn runtime_emits_beacons_in_tick_output() {
        ...
        assert!(peak > 0.0, "idle beacon should produce output, peak={}", peak);
    }
    ...
    fn runtime_clear_queue_removes_messages() {
        ...
        let t0 = Instant::now();
        let voice = vec![0.3f32; 4800];
        for i in 0..5 { rt.process_mic(&voice, true, t0 + Duration::from_millis(i * 100)); }
        let silence = vec![0.0f32; 4800];
        for i in 0..3 { rt.process_mic(&silence, false, t0 + Duration::from_millis(500 + i * 100)); }
