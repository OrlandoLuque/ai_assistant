...
use ai_assistant::{
    DatasetBuilder, DatasetConfig, DatasetFormat, DistillationStepType, TrajectoryCollector,
    TrajectoryDataset, TrajectoryOutcome, TrajectoryStep,
};
...
fn main() {
    ...
    collector
        .finish_trajectory(&tid, TrajectoryOutcome::Success { score: 0.95 })
        .unwrap();
    ...
    collector
        ...
        .unwrap();
    collector
        .finish_trajectory(&tid2, TrajectoryOutcome::Success { score: 0.90 })
        .unwrap();
        ...
        Ok(entries) => {
            ...
            for (i, entry) in entries.iter().enumerate() {
                println!(
                    "  Entry {}: input='{}' output='{}'",
                    i,
                    &entry.input[..entry.input.len().min(40)],
