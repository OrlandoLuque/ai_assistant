...
fn main() {
    ...
    println!(
        "  Object URL:     {}",
        s3_client.object_url("models/llama3.gguf")
    );
    ...
    println!(
        "    Body size:    {} bytes",
        put_req.body.as_ref().map(|b| b.len()).unwrap_or(0)
    );
    ...
    let drive_config =
        GoogleDriveConfig::new("ya29.example-access-token").with_folder("1ABC_folder_id");
    ...
    for (name, op) in &azure_ops {
        let req =
            AzureBlobRequest::new("myaccount", "aicontainer", "models/llama3.bin", op.clone());
        println!(
            "  {:<12} method={:<6} url={}",
            name,
            req.to_method(),
    ...
    println!(
        "  ListResult:  {} objects, has_next={}",
        list_result.objects.len(),
        list_result.next_page_token.is_some()
    );
    ...
    println!("\n  Azure Blob:");
    let req =
        azure_connector.build_request("myaccount", "container", "blob.bin", StorageOperation::Get);
    ...
    println!("\n  GCP Storage:");
    let req = gcp_connector.build_request(
        "my-project",
        "gcs-bucket",
        "object.bin",
        StorageOperation::Get,
