...
use ai_assistant::{
    BootstrapFewShot, CompiledPrompt, ExactMatch, FieldType, Signature, SignatureField,
};
...
fn main() {
    ...
    .add_input(SignatureField::new("text", "The text to analyze"))
    .add_output(
        SignatureField::new("sentiment", "The detected sentiment").with_type(FieldType::Text),
    )
    .add_output(
        SignatureField::new("confidence", "Confidence score 0-1").with_type(FieldType::Number),
    );
    ...
    println!(
        "Inputs: {:?}",
        sig.inputs.iter().map(|f| &f.name).collect::<Vec<_>>()
    );
    println!(
        "Outputs: {:?}",
        sig.outputs.iter().map(|f| &f.name).collect::<Vec<_>>()
    );
    ...
    println!(
        "{}",
        &compiled.system_prompt[..compiled.system_prompt.len().min(200)]
    );
    ...
    inputs.insert(
        "text".to_string(),
        "I love this product! It works great.".to_string(),
    );
